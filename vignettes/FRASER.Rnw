...
From \fraser{}2 on, only a single metric - the Intron Jaccard Index (Figure 
...
# to show result visualization functions for this tuturial, no cutoff used
res <- results(fds, padjCutoff=NA, deltaPsiCutoff=NA)
res
...
res_gene <- results(fds, aggregate=TRUE, padjCutoff=NA, deltaPsiCutoff=NA)
res_gene
@
...
sampleRes <- res[res$sampleID == "sample8"]
sampleRes
@
...
plotExpression(fds, type="jaccard", result=sampleRes[1])
