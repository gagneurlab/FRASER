...
From \fraser{}2 on, only one metric - the Intron Jaccard Index (Figure 
...
\begin{equation}
    J(D,A) = \frac{n(D,A)}{\sum_{A'} n(D,A') + \sum_{D'} n(D',A) + n(D) + n(A) - n(D,A)}\label{eq:jaccard}
\end{equation}
...
res_gene <- results(fds, aggregate=TRUE, padjCutoff=NA, deltaPsiCutoff=0.1)
res_gene
@
