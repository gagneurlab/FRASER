...
# filtering of lowly expressed junctions: This step is skipped here because
# it is not meaningful on the small example dataset. See the section on
# filtering below for more details
...
crucial. For some hints please refere to
[these slides](https://docs.google.com/presentation/d/1a7KZ6FXwVmGqF-FMAnz0QTk07WgzRWAbFhSNrUBspIQ/edit?pli=1).
...
fds <- loadFraseRDataSet(file=file.path(workingDir(fds), "savedObjects", 
                                        "ExampleAnalysis", "fds-object.RDS"))
...
fds <- fit(fds, q=3, type="psi5", correction="PCA-BB-Decoder", iterations=2)
@
