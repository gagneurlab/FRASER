...
<<quick faser guide all, echo=TRUE, eval=FALSE>>=
...
# load data
colData <- fread(system.file(
    "extdata", "sampleTable.tsv", package="FraseR", mustWork=TRUE
))
if(FALSE){
    colData[,bamFile:=paste0("inst/", bamFile)]
}

# count data
fds <- FraseRDataSet(colData=colData, parallel=bpparam())
dontWriteHDF5(fds) <- TRUE

fds <- countRNAData(fds, recount=TRUE)
fds
...
res <- results(fds, zScoreCutoff=2, padjCutoff=NA, deltaPsiCutoff=0.01)
res <- res[names(res) %in% c("sample1", "sample2", "sample3") &
    res$counts >= 10 & !is.na(res$zScore)]
res

plotVolcano(fds, sampleID="sample1", type="psi5", aggregate=TRUE)

plotExpectedVsObservedPsi(fds, result=res[1])
