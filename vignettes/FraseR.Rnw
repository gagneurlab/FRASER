...
To install and load \fraser{}, run the follwing:
...
<<quick faser guide, echo=TRUE, eval=FALSE>>=
...
# parallelization
register(MulticoreParam(workers=min(6, bpworkers())))
...
<<subset to filtered junctions, echo=TRUE, eval=FALSE>>=
# skipped for this tutorial because the example dataset is otherwise too small
...
<<intron sample correlation, echo=TRUE, eval=FALSE>>=
...
<<plot expression, echo=TRUE, eval=FALSE>>=
...
fds <- optimHyperParams(fds, "psi5", 
                             correction="PCA", # PCA used to speed it up
                             setSubset=50 # for this tutorial: only consider 
                                          # 50 most variable junctions for the 
...
<<result visualization, echo=TRUE>>=
...
plotQQ(fds, type="psi5", aggregate=TRUE, global=TRUE)
@
